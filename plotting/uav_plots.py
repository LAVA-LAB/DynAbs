...
def UAV_plot_2D(i_show, setup, args, regions, goal_regions, critical_regions,
    ...
    # Save figure
    filename = Path(setup.directories['outputFcase'], 'drone_trajectory')
...
def UAV_3D_plotLayout(setup, args, model, regions,
                      goal_regions, critical_regions, traces, spec, screenshot=True):
    '''
    ...
    UAVplot3d_visvis(setup, args, model, regions, goal_regions,
                     ...
                     cut_value=cut_value, screenshot=screenshot)


def UAVplot3d_visvis(setup, args, model, regions, goal_regions,
                     critical_regions, spec, traces, cut_value,
                     traces_to_plot=10, screenshot=True):
    '''
    Create 3D trajectory plots for the 3D UAV benchmark.
    Creates cross-section at given `cut_value`.

    Note: creating screenshots can sometimes create problems. If this happens you can
    srt the screenshot option to False.
    ...
    if 'outputFcase' in setup.directories:
        filename = Path(setup.directories['outputFcase'], 'UAV_paths_screenshot.png')

    else:
        filename = Path(setup.directories['outputF'], 'UAV_paths_screenshot.png')

    if screenshot:
        vv.screenshot(str(filename), sf=3, bg='w', ob=vv.gcf())
    app.Run()
