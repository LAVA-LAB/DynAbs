...
def reachability_plot(setup, results, mc=None):
    ...
    plt.plot(results['optimal_reward'], label='k=0', linewidth=1)
    ...
    # Save figure
    filename = Path(setup.directories['outputFcase'], 'reachability_probability')
...
def heatmap_3D_view(model, setup, spec, region_centers, results):
    ...
    if model.n > 2:
        ...
        # Save figure
        filename = Path(setup.directories['outputFcase'], '3d_reachability_k=0')
        for form in setup.plotting['exportFormats']:
            plt.savefig(filename.with_suffix('.' + str(form)), format=form, bbox_inches='tight')
...
def heatmap_2D(args, model, setup, c_tuple, spec, values, title='auto'):
    ...
    # Save figure
    filename = Path(setup.directories['outputFcase'], '2D_Heatmap_N=' + str(args.noise_samples))
    for form in setup.plotting['exportFormats']:
        plt.savefig(filename.with_suffix('.' + str(form)), format=form, bbox_inches='tight')
