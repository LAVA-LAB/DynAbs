...
def create_table(N, beta, kstep, trials, export=False, clopper_pearson=False):
    d = 1
    ...
    if clopper_pearson:
        # If clopper_pearson is true, use improved confidence level (by not dividing by N)
        beta_bar = beta / 2
    else:
    ...
    if export:
        if clopper_pearson:
            filename = 'input/CP_probabilityTable_N=' + str(N) + '_beta=' + str(beta) + '.csv'
        else:
