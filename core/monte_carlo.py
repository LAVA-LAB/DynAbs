...
class MonteCarloSim():
    ...
    def defineDisturbances(self):
        ...
        if self.args.nongaussian_noise:
            ...
            self.noise_idxs = np.random.randint(len(self.model.noise['samples']), size=(len(self.init_states), self.iterations, self.horizon))
    ...
    def _runIteration(self, s_abs, s_init, m):
        ...
        while k <= self.horizon:
            ...
            region_center = computeRegionCenters(x[k], self.spec.partition).flatten()

            if tuple(region_center) in self.partition['R']['c_tuple']:
                # Save that state is currently in region i
            ...
            if self.args.nongaussian_noise:
                ...
                x[k + 1] = x_hat + self.model.noise['samples'][self.noise_idxs[s_abs, m, k]]
