...
class Abstraction(object):
    ...
    def solve_iMDP(self):
        ...
        policy_file = pathlib.Path(self.setup.directories['outputFcase'], "PRISM_" + mode + '_policy.txt')
        vector_file = pathlib.Path(self.setup.directories['outputFcase'], "PRISM_" + mode + '_vector.csv')

        options = f' -exportstrat "{policy_file}" -exportvector "{vector_file}"'

        print(' --- Execute PRISM command for EXPLICIT model description')

        model_file = f'{self.mdp.prism_file}'
