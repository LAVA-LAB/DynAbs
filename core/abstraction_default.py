...
class abstraction_default(Abstraction):
    ...
    def _computeProbabilityBounds(self, tab, noise_samples):
        '''
        ...
        for a_idx, act in progressbar(self.actions['obj'].items(), redirect_stdout=True):
            ...
            if len(act.enabled_in) > 0:
                ...
                prob[a_idx], regions_list[a_idx], ignore[a_idx] = \
                    compute_intervals_default(self.args, self.spec.partition, self.partition, self.trans,
                                              len(noise_samples), successor_samples, successor_states, regions_list=cache)
    ...
    def define_probabilities(self, samples):
        '''
        ...
        if self.args.clopper_pearson:
            tableFile = self.setup.directories['base'] + '/input/CP_probabilityTable_N=' + \
                        str(len(samples)) + '_beta=' + \
        ...
        print(f' - With {len(samples)} noise samples')

        self.trans['prob'], self.trans['ignore'] = self._computeProbabilityBounds(tab, samples)
